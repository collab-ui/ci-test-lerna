...
import { Loading } from '@collabui/react';
...
class Button extends React.Component {
  ...
  componentDidMount() {
    ...
    (!ariaLabel && !ariaLabelledBy)
      &&
      console.warn('[@collabui/react] Button: Accessibility could be improved with ariaLabel');
  ...
  render() {
    ...
    const getSize = () => {
      ...
      if (!circle && !validButtonSize) {
        console.warn('[@collabui/react] Button: selected size is not supported for non-circular button. Size will default to 36');

        return '36';
      } else if (large) {
        console.warn('[@collabui/react] Button: large prop is deprecated and will be removed. Please use size prop.');
